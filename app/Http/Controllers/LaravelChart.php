...
class LaravelChart
{
    ...
    private function prepareData()
    {
        ...
        try {
            ...
            foreach ($conditions as $condition) {
                ...
                $query = $this->options['model']::when(isset($this->options['filter_field']), function ($query) {
                    ...
                    if (isset($this->options['filter_days'])) {
                        ...
                        switch ($this->options['filter_period']) {
                            case 'day':
                                $start = $this->options['filter_period_day'] ? : date('Y-m-d');
                                break;
                            case 'week':
                        ...
                        if (isset($start)) {
                            if($this->options['filter_period'] == 'day') {
                                return $query->where($this->options['filter_field'], 'like', '%'.$start.'%');
                            }
                ...
                ->when(isset($this->options['select_raw']), function($query) {
                    return $query->selectRaw($this->options['select_raw']);
                });
    ...
    private function validateOptions(array $options)
    {
        $rules = [
            ...
            'chart_type'            => 'required|in:line,bar,radar,polarArea,pie|bail',
            'filter_days'           => 'integer',
            'filter_period'         => 'in:day,week,month,year',
            'hidden'                => 'boolean'
        ];
    ...
    {
        return view('charts.html', ['options' => $this->options]);
    }
    ...
    {
        return view('charts.javascript', ['options' => $this->options, 'datasets' => $this->datasets]);
    }
