<div>
    <x-dropdown align="right" width="64">
        <x-slot name="trigger">
            <button
                class="inline-flex items-center px-2 py-1 mr-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                <div class="relative">
                    <div class="absolute left-5">
                        <p
                            ...
                            <svg wire:loading.class.remove="hidden" fill="white" class="hidden -mx-2" width="24"
                    ...
                    <svg height="30" width="30" class="bi bi-cart3 mt-2" viewBox="0 0 16 16">
                        <path
        ...
        <x-slot name="content">
                ...
                @foreach ($carts as $cart)
                    <x-dropdown-link :href="route('product.details', $cart->product->slug)" class="flex hover:bg-white focus:bg-white">
                        <div class="flex items-center bg-white overflow-hidden border-b -my-2">
                            <img class="w-10 h-10 bg-cover mx-2 border"
                            ...
                            <div class="w-3/4 px-0">
                                ...
                                <div class="flex justify-between mt-0">
                                    ...
                                    <div class="text-gray-700 font-bold text-sm mr-2">
                                        ${{ number_format($cart->price * $cart->units, 2) }}</div>
                                </div>
                            </div>
                            <a href="#" wire:click='clearCart({{ $cart->id }})' class="ml-2">
                                <svg fill="currentColor" class="bi bi-trash3 h-5 mr-3 my-2 text-red-800"
                                    viewBox="0 0 16 16">
                                    <path
            ...
            <div className="flex justify-center px-2 py-2 -mb-0.2 bg-green-800 text-white font-bold uppercase">
                <a href="{{ route('cart.details') }}"
                    class="flex py-2 mx-2 mt-2 bg-green-800 rounded text-gray-100 font-bold justify-center hover:bg-green-700 focus:bg-green-700">
                    {{ __('Go To Cart') }}
                </a>
            </div>
        </x-slot>
