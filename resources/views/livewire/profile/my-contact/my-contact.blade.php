    ...
    <x-card>
        <x-card-header  class="bg-green-700 -m-6">
            <div class="py-3 mx-4 text-lg text-white font-bold flex">
                <span class="ml-2">{{ __('My Contacts') }}</span>
        ...
        <x-card-body class="mt-6 mx-2">
            @if($contacts->count() ==  0)
                <div class="text-md text-gray-700">Add contacts to your list.</div>
            @else
                    ...
                    <body>
                        @foreach ($contacts as $key => $contact)
                            <x-tr>
                                ...
                                <x-td class="flex">
                                    <x-icon-edit wire:click="$emit('openModal', 'profile.my-contact.edit', [{{$contact->id}}])" class="mr-1 text-green-700 hover:text-green-400 hover:cursor-pointer rounded" width="24" height="24"/>
                                    <x-icon-trash wire:click="$emit('openModal', 'profile.my-contact.destroy', [{{$contact->id}}])" class="ml-1 text-red-700 hover:text-red-500 hover:cursor-pointer rounded" width="24" height="24" />
                                </x-td>
                            </x-tr>
                        @endforeach
                    </body>
            ...
            <div class="flex justify-end">
                <x-button wire:click="$emit('openModal', 'profile.my-contact.create')" type="button" class="mt-4 -mb-4 flex">
                    <svg fill="currentColor" class="bi bi-pencil-fill h-4 mr-1" viewBox="0 0 16 16">
