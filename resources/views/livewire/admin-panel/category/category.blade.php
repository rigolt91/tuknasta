                                        ...
                                        @hasrole('administrator')
                                            <x-td>
                                                <label class="relative inline-flex items-center mr-5 cursor-pointer">
                                                    <input type="checkbox" wire:click="setShow({{ $category->id }})" wire:loading.attr="disabled" class="sr-only peer" @if($category->show) checked @endif >
                                            ...
                                            <x-td class="flex float-right">
                                                <x-button wire:click="$emit('openModal', 'admin-panel.category.edit', [{{$category->id}}])" wire:loading.attr="disabled" class="flex items-center mx-1 disabled:opacity-60">
                                                    <x-icon-edit />
                                                    <span class="hidden sm:ml-2 sm:block">{{ __('Edit') }}</span>
                                                </x-button>

                                                <x-button-danger wire:click="$emit('openModal', 'admin-panel.category.destroy', [{{$category->id}}])" wire:loading.attr="disabled" class="flex items-center disabled:opacity-60">
                                                    <x-icon-trash />
