                ...
                <x-card-body>
                    <div class="pb-2 mb-2 -mt-2 font-bold border-b border-green-300">
                        ...
                        @hasrole('administrator')
                            <x-button-inline wire:click="$emit('openModal', 'admin-panel.product.create')" class="flex float-right -mt-2 ml-2">
                                <x-icon-file-plus />
                            ...
                            <tbody>
                                @foreach ($products as $key => $product)
                                    <x-tr wire:loading.class="opacity-60">
                                        <x-td>{{ ++$key }}</x-td>
                                        <x-td>
                                            <img src="{{ Storage::url($product->image) }}" class="w-12 h-12 bg-green-100 border border-green-400 rounded" alt="{{ $product->name }}">
                                        </x-td>
                                        ...
                                        @hasrole('administrator')
                                            <x-td>
                                                <label class="relative inline-flex items-center mr-5 cursor-pointer">
                                                    <input type="checkbox" wire:click="setShow({{ $product->id }})" wire:loading.attr="disabled" class="sr-only peer" @if($product->show) checked @endif >
                                            ...
                                            <x-td>
                                                <label class="relative inline-flex items-center mr-5 cursor-pointer">
                                                    <input type="checkbox" wire:click="setRecommend({{ $product->id }})" wire:loading.attr="disabled" class="sr-only peer" @if($product->recommend) checked @endif >
                                                    <div class="w-9 h-5 bg-gray-200 rounded-full peer peer-focus:ring-4 peer-focus:ring-green-300 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-green-600"></div>
                                                </label>
                                            </x-td>
                                            <x-td class="flex float-right">
                                                <x-button wire:click="$emit('openModal', 'admin-panel.product.edit', [{{$product->id}}])" wire:loading.attr="disabled" class="flex items-center mr-1 disabled:opacity-60">
                                                    <x-icon-edit />
                                                    <span class="hidden ml-2 sm:block">{{ __('Edit') }}</span>
                                                </x-button>

                                                <x-button-danger wire:click="$emit('openModal', 'admin-panel.product.destroy', [{{$product->id}}])" wire:loading.attr="disabled" class="flex items-center disabled:opacity-60">
                                                    <x-icon-trash />
