<div>
    <div class="m-2" wire:loading.class="opacity-60">
        <div class="flex h-full transition ease-in-out bg-white border border-gray-100 sm:rounded hover:shadow-xl hover:-translate-y-1 duration-450">
            <a href="{{ route('product.details', $slug) }}" class="bg-white rounded sm:w-4/12"  wire:loading.class="animation-pulse">
            ...
            <div class="w-8/12 mx-2 my-1">
                <div class="py-2 text-sm font-bold text-gray-800 border-b sm:text-md ">
                    {{ $name }}
                    <div>
                        <div wire:click='addProductCart({{ $product }})' wire:loading.attr='disabled' class="float-right w-6 h-6 -mt-6 cursor-pointer disabled:opacity-60">
                ...
                <div class="py-1 text-sm text-gray-700 sm:block">{{ substr($short_description, 0, 50) }}...</div>

                <div class="flex items-center">
                    <div class="font-bold text-gray-800 text-md">${{ number_format($price, 2) }}</div>
                    <div class="ml-2 text-sm font-bold text-gray-500">${{ number_format($previous_price, 2) }}</div>
                </div>
            </div>
        </div>
    </div>
